...
st.set_page_config(page_title="NASA AI | MASTER SERVER V25", page_icon="📡", layout="wide")

st.markdown("""
<style>
    @import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@400;900&family=Share+Tech+Mono&display=swap');
    
    /* মেইন ব্যাকগ্রাউন্ড - স্পেস গ্রেডিয়েন্ট */
    .stApp { 
        background: radial-gradient(circle at center, #00122e 0%, #000000 100%) !important; 
        color: #ffffff; 
        font-family: 'Share Tech Mono', monospace; 
    }
    
    /* নাসার অফিসিয়াল নিয়ন টাইটেল */
    .main-title { 
        color: #00f2ff; 
        text-align: center; 
        font-family: 'Orbitron', sans-serif; 
        font-size: 38px; 
        text-shadow: 0 0 35px #00f2ff, 0 0 10px #ffffff; 
        margin-top: -20px;
        margin-bottom: 20px;
    }
    ...
    /* পেমেন্ট কার্ড - বিকাশ কালার নিয়ন */
    .payment-card { 
    ...
    /* স্লাইডার এবং ইনপুট ফিল্ড ডিজাইন */
    .stSlider > div > div > div > div { background-color: #00f2ff !important; }
    .stTextInput > div > div > input { background-color: rgba(255,255,255,0.05) !important; color: white !important; border: 1px solid #00f2ff !important; }
    
</style>
""", unsafe_allow_html=True)

# ৩. সেশন এবং সিকিউরিটি ভেরিয়েবল
...
if st.session_state.hacker_mode:
    st.markdown('<div class="hacker-alert">⚠️ ILLEGAL ACCESS DETECTED!<br>DECRYPTING IP... NOTIFYING CYBER DIVISION.</div>', unsafe_allow_html=True)
    st.error("🚨 হ্যাকিং চেষ্টার কারণে আপনার ডিভাইসের তথ্য এবং আইপি ট্র্যাক করা হয়েছে।")
...
if not st.session_state.authenticated:
    ...
    # নাসার লোগো সরাসরি গেটওয়ের উপরে মাঝখানে এড করা হয়েছে
    st.markdown(f"""
        <div class="logo-container">
            <img src="https://upload.wikimedia.org/wikipedia/commons/e/e5/NASA_logo.svg" width="130">
        </div>
    """, unsafe_allow_html=True)
    ...
    with col2:
        if not st.session_state.payment_done:
            # ডিসকাউন্ট চেক
            ip_code = st.text_input("🛡️ Discount IP Code (ঐচ্ছিক):", placeholder="AKASH-XXXX")
            ...
            # বিকাশ/নগদ পেমেন্ট কার্ড (তোমার নাম্বার সহ)
            ...
            with st.form("pay_gate"):
                ...
                if st.form_submit_button("ভেরিফাই এবং সার্ভার আনলক করুন"):
                    if u_trx == GOD_MODE_KEY:
                        ...
                        st.rerun()
                    elif any(x in u_trx.lower() for x in ["hack", "bypass", "crack", "sql"]):
                        st.session_state.hacker_mode = True
                        st.rerun()
                    elif len(u_trx) >= 8:
                        # অটো ডিভাইস ট্র্যাকিং
                        dummy_ip = f"103.{random.randint(10,99)}.{random.randint(100,255)}." + str(random.randint(1,255))
                        detected_dev = random.choice(["iPhone 15 Pro", "Samsung S24 Ultra", "Xiaomi 14", "Pixel 8", "Vivo V30", "OnePlus 12"])
                        ...
                        st.success("TrxID ভেরিফাই হচ্ছে... দয়া করে অপেক্ষা করুন।")
                        time.sleep(2)
                        st.rerun()
                    else:
                        st.error("ভুল TrxID! সঠিক তথ্য দিন নতুবা আইপি ব্লক করা হবে।")
        else:
            with st.form("gmail_gate"):
                st.success("✅ পেমেন্ট সাকসেসফুল! জিমেইল কানেক্ট করুন।")
                ...
                if st.form_submit_button("ফাইনাল কানেক্ট"):
                    if "@gmail.com" in u_mail:
                        ...
                        st.error("সঠিক জিমেইল অ্যাড্রেস দিন।")

else:
    # ৬. এডমিন ও ইউজার প্যানেল (২১ কোটি পাওয়ার আপডেট)
    st.markdown(f"<h1 class='main-title'>SYSTEM CORE: {st.session_state.user_name}</h1>", unsafe_allow_html=True)
    
    if st.session_state.is_admin:
        st.info("🚀 পাওয়ার মোড: ২১০,০০০,০০০X | স্ট্যাটাস: সুপ্রিম এডমিন (আকাশ)")
        t1, t2, t3 = st.tabs(["🕵️‍♂️ ডিভাইস ও সেলস লগ", "🔑 কোড ম্যানেজার", "🚀 মাস্টার টুলস"])
        with t1:
            if os.path.exists(TRACKER_FILE):
                with open(TRACKER_FILE, "r", encoding="utf-8") as f:
                    st.markdown(f'<div style="background:#000; padding:15px; color:#00ff88; border:1px solid #00f2ff; height:400px; overflow-y:scroll; font-family:Share Tech Mono;">{"<br>".join(f.readlines()[::-1])}</div>', unsafe_allow_html=True)
        with t2:
            st.write(f"আজকের ডিসকাউন্ট আইপি: {st.session_state.generated_ip}")
            if st.button("লগ আউট"): 
                st.session_state.authenticated = False
                st.rerun()
        with t3:
            st.button("জিমেইল অটোমেশন")
            st.button("টিকটক ডাটা সিঙ্ক")
    else:
        st.success(f"অভিনন্দন {st.session_state.user_name}! ২১ কোটি পাওয়ার এক্সেস একটিভ।")
        st.chat_input("আপনার কমান্ড এখানে দিন...")
